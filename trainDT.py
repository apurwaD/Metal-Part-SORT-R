...
def treeTraversal(root):
	...
	if(root is not None):
		...
		# print("values ", root.value)
		# print("sampleLength ", root.sampleLength)
	
		#print("left",len(root.children[0]))
		#print("right",len(root.children[1]))
	...
	if(presentDepth == pruningDepth):
		if (node.left is not None and node.right is not None):
			if(node.left.left == None and node.right.right == None and node.left.right == None and node.right.left == None):
				# print('Pruning', node.sampleLength)
